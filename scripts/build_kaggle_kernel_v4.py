"""Assembles the Phase 3 ENSEMBLE pipeline (analog propagation fused with the
...
OUT_DIR = ROOT / "kaggle_kernel_v4"
COMPETITION = "enveda-CASMI26-molecule-id-mass-spectra"
KAGGLE_USERNAME = "isaackjoshua"
KERNEL_SLUG = "casmi26-phase-3-ensemble"
KERNEL_TITLE = "CASMI26 Phase 3 Ensemble"
...
def build_notebook() -> dict:
    cells = [
        _markdown_cell(
            ...
            "the 2.5M labelled training spectra), ensembled with a peak-level transformer over "
            "exact m/z and neutral losses. The MLP is fine-tuned on the "
...
f'"/kaggle/input/datasets/{MODEL_DATASET}/peak_model.pt"]\n\n'
            ...
            'print(f"ensemble of {len(MODEL_PATHS)} models loaded on {device}")\n\n'
            "try:\n"
...
def build_metadata() -> dict:
    return {
        ...
        "title": KERNEL_TITLE,
        "code_file": "casmi26_phase3_ensemble.ipynb",
        "language": "python",
...
def main() -> None:
    ...
    (OUT_DIR / "casmi26_phase3_ensemble.ipynb").write_text(json.dumps(build_notebook(), indent=1))
    (OUT_DIR / "kernel-metadata.json").write_text(json.dumps(build_metadata(), indent=2))
    print(f"Wrote {OUT_DIR}/casmi26_phase3_ensemble.ipynb and kernel-metadata.json")
